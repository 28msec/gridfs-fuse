...
{
    setLayout(layout1);
    WriterPtr wr(createWriter_static(getSystemOut()));
    setWriter(wr);
    Pool p;
...
{
      setLayout(layout1);
      WriterPtr wr(createWriter_static(target1));
      setWriter(wr);
      Pool p;
      WriterAppender::activateOptions(p);
}
...
WriterPtr ConsoleAppender::createWriter_static(const LogString& value) {
